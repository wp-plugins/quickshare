...
function cxnh_quickshare_add_defaults() {
    ...
    if( ! is_array( $tmp ) ) {
		...
		$arr = array(
			// general
			'plugin_version' => '1.5', // Currently tracks initially installed plugin version.
			'settingspage' => 'design',
...
// Prepare the media uploader & color pickers.
function cxnh_quickshare_admin_scripts() {
	// Must be running WordPress 3.5+ to use color pickers and image upload.
	wp_enqueue_media();
...
?>
	<style type="text/css">
		.icons .text-option,
		.icons .genericons-option,
		.icons .n-icons-option {
			display: none;
		}

		.genericons .text-option,
		.genericons .icons-option,
		.genericons .n-genericons-option {
			display: none;
		}

		.text .icon-option,
		.text .genericons-option,
		.text .n-text-option {
			display: none;
		}
	...
	<?php 
		if( cxnh_quickshare_getOption( 'everywhere', $options ) ) {
			echo '.display-option { display: none; }';
		}
		if( $options['settingspage'] == 'design' ) {
			echo '#quickshare_config { display: none; }';
		}
		if( $options['settingspage'] == 'config' ) {
			echo '#quickshare_design { display: none; }';
		}
	?>
	</style>
...
?>
	<style type="text/css">
		...
		<?php if( cxnh_quickshare_getOption( 'respond_small', $options ) ) { ?>
			@media only screen and (max-width: <?php echo cxnh_quickshare_getOption( 'respond_small', $options ); ?>px) {
		...
		<?php } if( cxnh_quickshare_getOption( 'respond_hide', $options ) ) { ?>
			@media only screen and (max-width: <?php echo cxnh_quickshare_getOption( 'respond_hide', $options ); ?>px) {
